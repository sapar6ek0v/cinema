...
const SerialsInfo = () => {
  ...
  useEffect(() => {
    const prom1 = axios(`${URL_BASE}/tv/${id}?api_key=${API_KEY}&language=en-US`).then(({ data }) => {
      setSerial(data);
    });

    const prom2 = axios(`${URL_BASE}/tv/${id}/videos?api_key=${API_KEY}&language=ru,en`).then(({ data }) => {
      setTrailer(data.results);
    });

    const prom3 = axios(`${URL_BASE}/tv/${id}/keywords?api_key=${API_KEY}&language=ru,en`).then(
      ({ data }) => {
        setKeyWord(data.results);
      }
    );

    const prom4 = axios(`${URL_BASE}/tv/${id}/recommendations?api_key=${API_KEY}&language=ru,en`).then(
      ({ data }) => {
        setRecommendation(data.results);
      }
    );

    Promise.all([prom1, prom2, prom3, prom4]).then(() => setLoading(false));
  }, [id]);
  ...
  return (
    <div key={serial.id} className="about-film">
          ...
          <Row>
            <div className="col-4 ">
              <div className="py-4 ">
                <div onClick={() => setModalImage(true)} className="position-relative">
                  <img
                    className="serials-bg film-info-img"
                    src={serial.poster_path ? `${PHOTO_BASE}${serial.poster_path}` : notFound}
                    alt="serials-bg"
                  />
            ...
            <div className="col-8">
              <div className="serials-bg-box">
                ...
                <div className="film-pod-title d-inline-block">
                  {serial.first_air_date.split('-').join('/')}
                </div>
                ...
                <div className="d-flex align-items-center">
                  ...
                  <span className="actor-link-block">
                    <a className="actor-link" href={serial.homepage}>
      ...
      <div
        className="sls-trl-crd-box"
        style={{
          background: `rgba(0,0,0,0.9) url(https://image.tmdb.org/t/p/original/${
            serial.backdrop_path || serial.poster_path
          }) center/cover`,
          backgroundBlendMode: 'darken',
        }}
      >

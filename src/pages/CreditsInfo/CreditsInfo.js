...
import MediaLinks from '../../components/Details/DetailsHeader/MediaLinks';
...
const CreditsInfo = () => {
  ...
  useEffect(() => {
    const prom1 = axios(`${URL_BASE}/person/${id}?api_key=${API_KEY}`).then(({ data }) => {
      setActor(data);
    });

    const prom2 = axios(`${URL_BASE}/person/${id}/external_ids?api_key=${API_KEY}`).then(({ data }) => {
      setMedia(data);
    });
        ...
        <Container>
          <div className="d-flex actors-box">
            ...
            <div>
              ...
              <MediaLinks id={id} type="person" homepage={''} />
              <div className="d-flex mb-4 actors-buttons">
                <button className="link-button" value="overview" onClick={() => toggleContent('overview')}>
                  overview
                </button>
                <button
                ...
                <button className="link-button" value="media" onClick={() => toggleContent('media')}>
                  Media
                </button>
                <button className="link-button" value="more" onClick={() => toggleContent('more')}>
                  + More
                </button>
              </div>
      ...
      <div className="actors-film-page">
        <Container>
          <div>
            ...
            <div>
              <button
                onClick={() => toggleContent('movie')}
                value="movie"
                className="content-btn credits-btn-item"
                type="button"
              >
                #Movie
              </button>
              <button
                onClick={() => toggleContent('tv')}
                value="tv"
                className="content-btn credits-btn-item"
                type="button"
              >

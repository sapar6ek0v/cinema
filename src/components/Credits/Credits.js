...
const Credits = () => {
  ...
  useEffect(() => {
    axios(`${URL_BASE}/movie/${id}/credits?api_key=${API_KEY}&language=ru`).then(({ data }) => {
    ...
    <div className="credits-box">
      <Container>
        ...
        <div className="px-3 py-4 d-flex overflow-auto cr-pd">
                ...
                <Link to={`/credits-info/${people.id}`} className="text-decoration-none ">
                  <div className="photo-box">
                    <img
                      ...
                      src={people.profile_path ? `${PHOTO_BASE}${people.profile_path}` : anonym}
                      alt={people.name}
                    />
                  </div>
          ...
          {actors < credits.length && (
            <div className="d-flex align-items-center">
              <button onClick={() => setActors(actors + 10)} className="credits-btn" type="button">
                Смотреть ещё...{' '}
                <span className=" credits-btn">

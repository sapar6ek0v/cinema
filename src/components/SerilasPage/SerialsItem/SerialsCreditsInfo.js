...
const SerialsCreditsInfo = () => {
  ...
  useEffect(() => {
    axios(`${URL_BASE}/tv/${id}/aggregate_credits?api_key=${API_KEY}&language=en-US`).then(({ data }) => {
      setCasts(data.cast);
    });
  }, [id]);
            ...
            <div key={actors.id} className="photo-page ">
              <Link to={`/credits-info/${actors.id}`} className="text-decoration-none ">
                ...
                <div className="">
                  ...
                  <div className="px-2 hover">
                    {actors.roles?.map((it) => {
                      return <div>{it.character}</div>;
                    })}
                  </div>
                </div>
              </Link>
